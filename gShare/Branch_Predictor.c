...
Branch_Predictor *initBranchPredictor(predictor_args *config)
{
    ...
    if (!strcmp(config->predictor_type, "two_bit_local")) {
        ...
        assert(checkPowerofTwo(branch_predictor->local_predictor_size));

        branch_predictor->local_predictor_mask = branch_predictor->local_predictor_size - 1;

        // Initialize sat counters
        branch_predictor->local_counters =
            (Sat_Counter *)malloc(branch_predictor->local_predictor_size * sizeof(Sat_Counter));

        int i = 0;
        for (i; i < branch_predictor->local_predictor_size; i++)
        {
...
bool predict(Branch_Predictor *branch_predictor, Instruction *instr, predictor_args *config)
{
    ...
    if (!strcmp(config->predictor_type, "two_bit_local")) {
        ...
        // Step one, get branch address low-order bits.
        unsigned branch_address_bits = getIndex(branch_address,
                                            branch_predictor->global_history_mask);

        // Step two, get global history low-order bits.
        unsigned global_history_bits = 
            branch_predictor->global_history & branch_predictor->global_history_mask;

        // Step three, apply bit-wise XOR on both branch address bits and global history bits to obtain gshare prediction.
        unsigned global_predictor_idx = 
            branch_address_bits ^ global_history_bits;

        bool global_prediction = 
            getPrediction(&(branch_predictor->global_counters[global_predictor_idx]));

        bool prediction_correct = global_prediction == instr->taken;
        // Step four, update counters
        if (instr->taken)
        {
            incrementCounter(&(branch_predictor->global_counters[global_predictor_idx]));
        }
        ...
        // Step five, update global history register
        branch_predictor->global_history = branch_predictor->global_history << 1 | instr->taken;
        // exit(0);
        //
        return prediction_correct;
